...
//   const imgs = props.images.map(img => <img src={"http://localhost:8000/"+img}/>)
//   if(fruit){
//     return (
...
class SingleFruit extends Component {
  render() {
    ...
    const fruitImages = this.props.images.map(img => <img key={img._id} src={"http://localhost:8000/"+img.image} />)
    //const imgs = this.props.images.map(img => <img src={"http://localhost:8000/uploads/"+img}/>)
    //console.log(imgs)
    if(fruit){
      return (
        <div className="single-fruit">
